...
//! (runtime checked).
//!
//! They can be registered and retrieved via
//! [`crate::registry::AccessorRegistry`].
...
impl<S, T> Accessor<S, T> {
    ...
    pub const fn ref_fn(&self) -> fn(&S) -> &T {
        self.ref_fn
    }

    pub const fn mut_fn(&self) -> fn(&S) -> &T {
        self.ref_fn
    }
...
unsafe impl Send for UntypedAccessor {}
unsafe impl Sync for UntypedAccessor {}
